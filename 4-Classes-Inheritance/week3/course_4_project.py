...
# version of the game Wheel of Fortune. Here are the rules of our game:
...
#    Players see the category and an obscured version of the phrase where every 
#    alphanumeric character in the phrase starts out as hidden 
#    (using underscores: _):
#            Category: Artist & Song
...
#                            -Vowels (a, e, i, o, u) cost $250 to guess and can’t be guessed if the player 
#                             doesn’t have enough money. All other letters are “free” to guess
...
#                             already had)
#                            -If the letter does appear in the phrase, the user keeps their turn. 
...
#            If the wheel lands on “lose a turn”, the player loses their turn and the game moves on 
#            to the next player
...
#First, let’s learn about a few functions and methods that we’ll use along 
# the way to do this project. There are no questions to answer in the next 
# four active code windows. They are just here to introduce you to some functions 
# and methods that you may not be aware of. The active code window that starts 
# with “Part A” is where you are first asked to complete code.

#The time.sleep(s) function (from the time module) delays execution of the 
# next line of code for s seconds. You’ll find that we can build a little 
# suspense during gameplay with some well-placed delays. 
# The game can also be easier for users to understand if not everything happens 
# instantly.
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self,guessed):
        ...
        compPrizeMoney=self.prizeMoney;
        listAllowedLetters=[]

        for letter in LETTERS:
            if letter not in guessed:
                if letter not in VOWELS:
                    listAllowedLetters.append(letter)
                elif letter in VOWELS and compPrizeMoney > VOWEL_COST:
                    listAllowedLetters.append(letter)
                elif letter in VOWELS and compPrizeMoney > VOWEL_COST:
                    print(letter,compPrizeMoney)
                    
        return listAllowedLetters
        
    def getMove(self, category, obscuredPhrase, guessed):
        
        listPossibleLetters = self.getPossibleLetters(guessed)

        if listPossibleLetters == []:
            #print(lett)
            return 'pass'
        ...
        if self.smartCoinFlip():
          for i in self.SORTED_FREQUENCIES[::-1]:
              if i in listPossibleLetters:
                  return i
        else:
            return random.choice(listPossibleLetters)  

#

#Putting it together: Wheel of Python
#Below is the game logic for the rest of the “Wheel of Python” game. 
# We have implemented most of the game logic. Start by carefully reading this code and double checking 
# that it all makes sense. Then, paste your code from the previous code window in the correct places below.
...
#says that we should be able to run our program for ms milliseconds before it gets stopped automatically.
sys.setExecutionLimit(ms)  
