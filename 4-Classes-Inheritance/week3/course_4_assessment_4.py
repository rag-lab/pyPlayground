...
# Olympics. However, this code currently does not work. Correctly add try/except 
# clause in the code so that it will correctly populate the list, country_gold,
...
# with the remainder of 36 divided by each number in numb. For example, 
# the first element should be 0, because 36/6 has no remainder. 
...
# and the string ‘Continent does not have 3 countries’ is appended to countries 
...
# Use try/except so that the code will run properly. If the sport is not in 
# the dictionary, ppl_play, add it in with the value of 1.
...
# some dictionaries. Insert a try/except so that the code passes. If the key 
# is not there, initialize it in the dictionary and set the value to zero.
di = [{"Puppies": 17, 'Kittens': 9, "Birds": 23, 'Fish': 90, "Hamsters": 49}, {"Puppies": 23, "Birds": 29, "Fish": 20, "Mice": 20, "Snakes": 7}, {"Fish": 203, "Hamsters": 93, "Snakes": 25, "Kittens": 89}, {"Birds": 20, "Puppies": 90, "Snakes": 21, "Fish": 10, "Kittens": 67}]
total = 0
for diction in di:
